...
function DemoApp() {
  ...
  useEffect(() => {
    const update = () => {
      const nextRoute = window.location.hash || '#/decks';
      // React batches both updates: never clear setup mode while rendering the old route.
      setRoute(nextRoute);
      setNewDeckId((id) => nextRoute === `#/decks/${id}` ? id : null);
    };
    window.addEventListener('hashchange', update);
    return () => window.removeEventListener('hashchange', update);
  }, []);
  const match = route.match(/^#\/decks\/(\d+)$/);
  const showList = () => { window.location.hash = '/decks'; };
  return <div className="ws4">
    ...
    {match ? <DeckDetailPage key={match[1]} api={api} deckId={match[1]} startAdding={String(newDeckId) === match[1]} onBack={showList} />
      : route === '#/decks' || route === '#' ? <DeckListPage api={api}
        onOpenDeck={(id) => { window.location.hash = `/decks/${id}`; }}
